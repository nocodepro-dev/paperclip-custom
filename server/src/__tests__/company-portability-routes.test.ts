...
describe("company portability routes", () => {
  ...
  it("allows CEO agents to use company-scoped export preview routes", async () => {
    ...
    mockCompanyPortabilityService.previewExport.mockResolvedValue({
      rootPath: "paperclip",
      manifest: { agents: [], skills: [], projects: [], issues: [], pipelines: [], envInputs: [], includes: { company: true, agents: true, projects: true, issues: false, skills: false, pipelines: false }, company: null, sidebar: null, schemaVersion: 1, generatedAt: new Date().toISOString(), source: null },
      files: {},
      fileInventory: [],
